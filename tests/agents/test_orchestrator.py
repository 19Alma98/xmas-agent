...
def test_orchestrator_search_web_recipes(mock_vector_store_class):
    ...
    with patch.object(orchestrator.recipe_researcher, 'search_web_recipes', return_value="Test results") as mock_search:
...
def test_orchestrator_search_web_recipes_without_dietary(mock_vector_store_class):
    ...
    with patch.object(orchestrator.recipe_researcher, 'search_web_recipes', return_value="Results") as mock_search:
...
@patch('src.agents.orchestrator.RecipeLoader')
@patch('src.agents.orchestrator.RecipeVectorStore')
def test_orchestrator_reload_recipes_with_json_file(mock_vector_store_class, mock_loader_class):
...
def test_run_return_type_and_structure(mock_ensure, mock_vector_store_class):
    ...
    with patch.object(orchestrator.info_checker, 'extract_preferences', return_value=mock_preferences_result), \
         patch.object(orchestrator.appetizer_agent, 'search', return_value=mock_recipe_result), \
         patch.object(orchestrator.main_dish_agent, 'search', return_value=mock_recipe_result), \
         patch.object(orchestrator.second_plate_agent, 'search', return_value=mock_recipe_result), \
         patch.object(orchestrator.dessert_agent, 'search', return_value=mock_recipe_result), \
         patch.object(orchestrator.menu_creator, 'create_menu', return_value=mock_menu_result):
...
def test_run_calls_all_agents(mock_ensure, mock_vector_store_class):
    ...
    with patch.object(orchestrator.info_checker, 'extract_preferences', return_value=mock_preferences_result) as mock_info, \
         patch.object(orchestrator.appetizer_agent, 'search', return_value=mock_recipe_result) as mock_app, \
         patch.object(orchestrator.main_dish_agent, 'search', return_value=mock_recipe_result) as mock_main, \
         patch.object(orchestrator.second_plate_agent, 'search', return_value=mock_recipe_result) as mock_second, \
         patch.object(orchestrator.dessert_agent, 'search', return_value=mock_recipe_result) as mock_dessert, \
         patch.object(orchestrator.menu_creator, 'create_menu', return_value=mock_menu_result) as mock_menu:
...
def test_run_streaming_yields_correct_structure(mock_ensure, mock_vector_store_class):
    ...
    with patch.object(orchestrator.info_checker, 'extract_preferences', return_value=mock_preferences_result), \
         ...
         patch.object(orchestrator.second_plate_agent, 'search', return_value=mock_recipe_result), \
         patch.object(orchestrator.dessert_agent, 'search', return_value=mock_recipe_result), \
         patch.object(orchestrator.menu_creator, 'create_menu_streaming', return_value=iter([mock_step])):
        ...
        for result in results:
            ...
            assert result["type"] in ["status", "progress", "warning", "step", "complete"]
...
def test_run_with_native_agents_return_structure(mock_ensure, mock_vector_store_class):
    ...
    with patch.object(orchestrator.info_checker, 'extract_preferences', return_value=mock_preferences_result), \
         patch.object(orchestrator.menu_creator, 'create_menu_with_agents', return_value=mock_menu_result):
...
@patch('src.agents.orchestrator.RecipeVectorStore')
@patch.object(ChristmasMenuOrchestrator, '_ensure_recipes_loaded')
def test_run_with_native_agents_calls_menu_creator(mock_ensure, mock_vector_store_class):
    ...
    with patch.object(orchestrator.info_checker, 'extract_preferences', return_value=mock_preferences_result), \
         patch.object(orchestrator.menu_creator, 'create_menu_with_agents', return_value=mock_menu_result) as mock_create:
